...
try {
  ...
  report.checks.passwordFreeEntry = session.unlocked === true;
  await expect(page.getByLabel("Room passphrase")).toHaveCount(0);
  report.checks.wandEnlarged = await page.locator(".wand-rest").evaluate(node => parseFloat(getComputedStyle(node).width) >= 311);
  report.checks.magicOutline = await page.locator(".wand-magic").count() === 1;
  await expect(
  ...
  await browser.close();
  await writeFile(
    "../docs/wand-2026-09-22/hosted-check.json",
